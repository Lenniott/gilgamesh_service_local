...
curl -X POST "http://localhost:8500/process" \
  -H "Content-Type: application/json" \
  -d '{
    "urls": ["https://www.instagram.com/p/DJRiA-gI2GT/?igsh=bXI4MWJzNnhhMm5t"],
    "encode_base64": false,
    "cleanup_temp": false
  }'
echo -e "\n\n"
...
curl -X POST "http://localhost:8500/process" \
  -H "Content-Type: application/json" \
  -d '{
    "urls": ["https://www.instagram.com/reel/DJVTMdRxRmJ/?igsh=eHJtN214OWpuMnIx"],
    "encode_base64": false,
    "cleanup_temp": false
  }'
echo -e "\n\n"
...
curl -X POST "http://localhost:8500/process" \
  -H "Content-Type: application/json" \
  -d '{
    "urls": ["https://youtube.com/shorts/izeO1Vpqvvo?si=blnnaTO0uYEe5htC"],
    "encode_base64": false,
    "cleanup_temp": false
  }'
echo -e "\n\n"
